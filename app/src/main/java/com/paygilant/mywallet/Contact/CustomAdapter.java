...
public class CustomAdapter extends BaseAdapter {
    ...
    public CustomAdapter(Context context, ArrayList<ContactModel> contactModelArrayList) {
        ...
        this.arraylist.addAll(contactModelArrayList);
    }
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            contactModelArrayList.addAll(arraylist);
        } else {
            for (ContactModel contactModel : arraylist) {
                if (contactModel.getName().toLowerCase(Locale.getDefault()).contains(charText)) {
